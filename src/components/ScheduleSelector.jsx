...
const ScheduleSelector = () => {
  ...
  const handleSectionChange = (e) => {
    ...
    const firstDayOfNewSection = scheduleData.sections.find(
      (s) => s.section_number === newSection
    ).schedule[0].day;
  ...
  const getAvailableDays = () => {
    const section = scheduleData.sections.find(
      (s) => s.section_number === selectedSection
    );
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8 animate-fade-in-up">{renderSchedule()}</div>
      </div>
    </div>
  );
};
